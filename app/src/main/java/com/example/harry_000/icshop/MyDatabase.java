...
public class MyDatabase extends SQLiteAssetHelper {
    ...
    public List<Store> getAllBusiness() {
        ...
        c.moveToFirst();
        while (!c.isAfterLast()) {
            Store store = cursorToBusiness(c);
            Store.add(store);
            c.moveToNext();
        }
    ...
    private Store cursorToBusiness(Cursor cursor) {
...
//search activity
public List<Store> getStoresBySearch(String key) {
    List<Store> Stores = new ArrayList<Store>();
    ...
    Cursor c = db.rawQuery(sqlState, new String[]{});
    c.moveToFirst();
    while (!c.isAfterLast()) {
        Store store = cursorToStore(c);
        Stores.add(store);
    ...
    public List<Brand> getBrandsBySearch(String key) {
    ...
    public List<Store> getStoresByProductCategory(String SubCategoryID) {
        List<Store> Stores = new ArrayList<Store>();
